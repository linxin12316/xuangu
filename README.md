...
A 股每日选股工具：每个交易日盘前 08:27 + 盘后 16:05 自动推送到你的微信。
...
| 时间（北京）| 任务 | 输出 |
|---|---|---|
| 08:27 | 盘前选股 | 候选股 Top 5（十维评分、止损建议、所属行业、连续上榜标记） |
...
3. 右上角点 **Run workflow** → 勾选 `force` → 绿色按钮
4. 等待 3-5 分钟看是否绿勾
5. 微信收到一条「📈 选股报告」= 配置完成 ✅
...
**Q: 早 8 点没收到推送？**
A: GitHub schedule 对私有仓库的整点（:00、:30）触发会被丢弃，已避开。
   如果还是没收到：
   - 看 Actions 页面有没有失败记录
   - 失败时 main.py 会主动推一条 ❌ 异常通知，没收到说明 schedule 没触发
   - 终极兜底：手动到 Actions 页面 Run workflow 触发一次
